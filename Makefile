...
build_linux: 
	...
	env GOOS=darwin GOARCH=amd64 go build -o ${BUILD_DIR}/${BINARY}_linux_x86-64 ${MAIN_PATH}

	@echo "Compiling for [${COLOR_GREEN}Linux${COLOR_RESET}] (${COLOR_YELLOW}ARM64${COLOR_RESET})"
	env GOOS=darwin GOARCH=arm64 go build -o ${BUILD_DIR}/${BINARY}_linux_arm64 ${MAIN_PATH}
 
build_windows: 
	@echo "Compiling for [${COLOR_GREEN}Windows${COLOR_RESET}] (${COLOR_YELLOW}x86-64${COLOR_RESET})"
	env GOOS=darwin GOARCH=amd64 go build -o ${BUILD_DIR}/${BINARY}_widnows_x86-64 ${MAIN_PATH}

	@echo "Compiling for [${COLOR_GREEN}Windows${COLOR_RESET}] (${COLOR_YELLOW}ARM64${COLOR_RESET})"
	env GOOS=darwin GOARCH=arm64 go build -o ${BUILD_DIR}/${BINARY}_windows_arm64 ${MAIN_PATH}
